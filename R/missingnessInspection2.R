...
#
#' @export
missingnessInspection2 <- function(jaspResults, dataset, options) {

  dataset <- .missingnessInspectionReadData(dataset, options)

    .patternPlot(jaspResults, dataset, options)
    .fluxPlot(jaspResults, dataset, options)
    .observedPlot(jaspResults, dataset, options)
    .missingPlot(jaspResults, dataset, options)

  return()
}

.patternPlot <- function(jaspResults, dataset, options) {
  if (!options[["patternPlots"]])
    return()

  if (is.null(jaspResults[["plotPattern"]])) {
    jaspResults[["plotPattern"]] <- createJaspContainer(gettext("Missing Data Pattern"))
    subcontainer <- jaspResults[["plotPattern"]]
  } else {
    subcontainer <- jaspResults[["plotPattern"]]
  }

  patternPlot <- createJaspPlot(title = title, width = 600, height = 420)
  patternPlot$dependOn(optionContainsValue = list(variables = variables))
  patternPlot$plotObject <- .createPatternPlot(dataset, options)
...
# Read in the dataset
.missingnessInspectionReadData <- function(dataset, options) {
  variables <- unlist(options[["variables"]])
  if (is.null(dataset)) {
    dataset <- .readDataSetToEnd(all.columns = variables)
  }
  return(dataset)
}
...
.patternPlot <- function(jaspResults, dataset, options) {
  ...
  mockPlot <- createJaspPlot(title = "Missing Data Pattern", height = 320, width = 480)
